        ...
        <section class="section mt-2">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form class="settings-form" method="POST" action="/employee/{{ $employees->id }}"
                        ...
                        <div class="row">
                            <div class="col-6">
                                <div class="mb-3">
                                    ...
                                    <input class="form-control @error('photo') is-invalid @enderror" name="photo"
                                        type="file" id="formFile" value="{{ $employees->photo }}">
                                    @error('photo')
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control @error('alamat') is-invalid @enderror" name="alamat"
                                        placeholder="Masukan Alamat Anda Disini..." rows="10">{{ $employees->alamat }}</textarea>
                                    @error('alamat')
                                        <div class="invalid-feedback text-danger">
                                            {{ $message }}
                                        </div>
                            ...
                            <div class="col-6">
                                    ...
                                    <select id="id_users" name="id_users" class="form-select">
                                        @foreach ($users as $user)
                                            <option value="{{ $user->id }}"
                                                {{ $user->id === $employees->id_users ? 'selected' : '' }}>
                                                {{ $user->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group row mt-2">
                                    <label for="id_divisis" class="col-4 col-form-label">Divisi</label>
                                    <select id="id_divisis" name="id_divisis" class="form-select">
                                        @foreach ($divisi as $div)
                                            <option value="{{ $div->id }}"
                                                {{ $div->id === $employees->id_divisis ? 'selected' : '' }}>
